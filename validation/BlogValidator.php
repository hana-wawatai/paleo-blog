...
final class BlogValidator {
    ...
    /**
     * Validate the given {@link Blog} instance.
     * @param Blog $blog {@link Blog} instance to be validated
     * @return array array of {@link Error} s
     */
    public static function validate(Blog $blog) {
        $errors = array();
        if (!$blog->getTitle()) {
            $errors[] = new Error('title', 'Title is Empty or invalid.');
        }
                if (!$blog->getDescription()) {
            $errors[] = new Error('description', 'Description is empty or invalid.');
        }
        if (!$blog->getContent()) {
            $errors[] = new Error('content', 'Content is empty or invalid.');
        }
        return $errors;
    }
    ...
    private static function isValidStatus($status) {
        return in_array($status, Blog::allStatuses());
    }

    private static function isValidPriority($priority) {
        return in_array($priority, Blog::allPriorities());
    }
